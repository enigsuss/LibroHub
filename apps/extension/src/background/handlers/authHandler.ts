type KakaoLoginResponse = { success: true } | { success: false; error: string };

export const handleKakaoLogin = async (sendResponse: (res: KakaoLoginResponse) => void) => {
  const REDIRECT_URI = chrome.identity.getRedirectURL();
  console.log(REDIRECT_URI);
  const REST_API_KEY = import.meta.env.VITE_KAKAO_REST_API_KEY;
  if (!REST_API_KEY) {
    sendResponse({ success: false, error: 'Kakao REST API Key is not defined' });
    return;
  }
